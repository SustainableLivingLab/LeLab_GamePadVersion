...
def setup_calibration_files(leader_config: str, follower_config: str):
    ...
    `leader_config` may be empty (gamepad input mode has no leader arm); in that
    case the leader lookup/copy is skipped entirely and `leader_config_name` is "".
    """
    # Extract config names from file paths (remove .json extension)
    leader_config_name = os.path.splitext(leader_config)[0] if leader_config else ""
...
def get_robot_record(name: str) -> dict | None:
    ...
    try:
        ...
        return None
    # Ensure all expected fields exist (forward/back compat). Records saved
    # before gamepad support have no "input_mode" key -- default to "leader"
    # so existing records keep working unchanged.
...
def is_robot_record_clean(record: dict) -> bool:
    """
    A record is 'clean' when its operational fields are populated AND the
    referenced calibration file(s) exist on disk. Cameras are optional and don't
    affect cleanliness.

    Gamepad input mode has no leader arm, so only the follower port/config are
    required and checked; leader mode requires both sides, as before.
    """
    if not record:
        return False
    ...
    return os.path.exists(leader_path)
