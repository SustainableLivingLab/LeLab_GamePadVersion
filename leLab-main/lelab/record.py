...
def create_record_config(request: RecordingRequest) -> RecordConfig:
    ...
    # Create teleop config. Gamepad mode has no leader arm/port.
    teleop_config = (
        GamepadSO101TeleopConfig()
        if request.input_mode == "gamepad"
...
def handle_recording_status() -> dict[str, Any]:
    ...
    # logger.info() only (no print()) here: a raw print() of this line's emoji
    # crashes with UnicodeEncodeError on a Windows console using the legacy
    # cp1252 codepage (colorama's console wrapper doesn't re-encode to UTF-8),
    # which took the whole request down with a 500 instead of just logging.
    if session_ended:
        if current_phase == "error":
            logger.info(
...
def record_with_web_events(cfg: RecordConfig, web_events: dict) -> LeRobotDataset:
    ...
    if isinstance(cfg.teleop, GamepadSO101TeleopConfig):
        teleop = GamepadSO101Teleop(cfg.teleop)
    else:
        teleop = make_teleoperator_from_config(cfg.teleop) if cfg.teleop is not None else None
    gamepad_mode = isinstance(teleop, GamepadSO101Teleop)
    ...
    try:
        ...
        if gamepad_mode:
            try:
                logger.info("🔧 TELEOP CONNECTION: Attempting to connect gamepad...")
                teleop.connect()
                logger.info("✅ TELEOP CONNECTION: Gamepad connected successfully")
            except Exception as e:
                logger.error(f"❌ TELEOP CONNECTION: Failed to connect gamepad: {e}")
                raise
        ...
        while saved_episodes < cfg.dataset.num_episodes:
            ...
            logger.info(
                f"🎬 Starting recording phase for episode {current_episode}/{cfg.dataset.num_episodes}"
            )
            ...
            if web_events["rerecord_episode"]:
                ...
                logger.info(
                    f"Re-recording episode {current_episode} (episode number stays the same)"
                )
            ...
            if web_events["stop_recording"]:
                logger.info("🛑 Recording manually stopped by user")
                break
        ...
        current_phase = "completed"
        phase_start_time = None
        logger.info("🏁 Recording session completed - all episodes finished")
