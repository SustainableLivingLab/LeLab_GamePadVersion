...
interface RobotRecord {
  name: string;
  input_mode: string; // "leader" or "gamepad"
  leader_port: string;
...
const Calibration = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Gamepad robots have no leader arm to calibrate -- always follower.
      // Otherwise default to the first incomplete side (leader, then follower).
      const defaultDevice =
        r.input_mode === "gamepad"
          ? "robot"
          : !r.leader_config
          ? "teleop"
          : !r.follower_config
